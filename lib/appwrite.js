...
// GALLERIES FUNCTIONS
// Updated Create a new gallery using the 'users' relationship field (single user)
export const createGallery = async ({ title, thumbnail, assets, assetType, userId }) => {
  try {
    const thumbnailUrl = await uploadFile(thumbnail, 'image'); // Upload thumbnail

    // Upload all selected assets (either images or videos)
    ...
    const newGallery = await databases.createDocument(
      ...
      ID.unique(),
      {
        title,
        thumbnail: thumbnailUrl,
        [assetType === 'image' ? 'images' : 'videos']: assetUrls, // Store assets as images or videos
        users: userId, // Pass the userId directly, without array
      }
    );
...
export const addImagesToGallery = async (galleryId, imageIds) => {
  try {
...
export const addVideosToGallery = async (galleryId, videoIds) => {
  try {
    const gallery = await databases.updateDocument(
      config.databaseId,
      config.galleriesCollectionId,
...
export const getLatestGalleries = async () => {
  try {
    ...
    return sortedGalleries.slice(0, 7);
  } catch (error) {

...
export const uploadFile = async (file, type) => {
  ...
  console.log('File being uploaded:', asset);
...
export const createGallery = async ({ title, thumbnail, assets, assetType, userId }) => {
  try {
    // Log before uploading the thumbnail
    console.log('Uploading thumbnail:', thumbnail.uri);
    ...
    const assetUrls = await Promise.all(assets.map(async (asset) => {
      ...
      const assetUrl = await uploadFile(asset, assetType);
      console.log('Uploaded asset URL:', assetUrl);
      return assetUrl;
    }));

    console.log('All assets uploaded successfully:', assetUrls);
    ...
    const newGallery = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        users: userId,
        eventDate, // Add the event date to the gallery
      }
    );
...
export const deleteGallery = async (collectionId, documentId, imageUrls = [], videoUrls = [], thumbnailUrl) => {
  try {
    // Delete associated images, videos, and thumbnail from storage
    const allUrls = [...imageUrls, ...videoUrls];

    if (thumbnailUrl) {
      allUrls.push(thumbnailUrl); // Add thumbnail URL to the list
    }
